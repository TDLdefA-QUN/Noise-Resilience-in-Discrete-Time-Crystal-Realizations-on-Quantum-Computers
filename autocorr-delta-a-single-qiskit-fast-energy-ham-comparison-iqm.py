...
parser.add_argument("--noise_prob",type=float, default=0.1, help="noise probability")
...
if args.use_fakebackend == 1:
    ...
    noise_model = NoiseModel.from_backend(IQMFakeGarnet())
else:
...
def qc_qiskit(initial_state, L, g, hs, phis, t, qubit, echo=False, noise_model=None, hamiltonian_type="full"):
    ...
    # Use AerSimulator for IQM device simulation
    # For real IQM device, you would replace this with the actual IQM backend
    ...
    results = estimator.run([(circ_tnoise, hamiltonian)],precision=1/np.sqrt(1024)).result()
    expval = results[0].data.evs
    print(f"Expectation value for {hamiltonian_type} at time {t} (echo={echo_str}): {expval}")
...
# IQM device simulation - no noise model needed for real device
state = initial_state

print("\n" + "="*50)
print("Running IQM device simulations...")
print("="*50)

# For IQM device, we don't apply artificial noise models
# The device will have its own inherent noise characteristics
iqm_noise_model = None  # No artificial noise model for real device

# Run simulation with only Z terms (IQM)
print("Computing Z terms only...")
energy_z_only_iqm = get_instances(state, echo=False, noise_model=iqm_noise_model, hamiltonian_type="z_only")
av_energy_z_only_iqm = np.mean(energy_z_only_iqm, axis=0)

# Run simulation with only ZZ terms (IQM)
print("Computing ZZ terms only...")
energy_zz_only_iqm = get_instances(state, echo=False, noise_model=iqm_noise_model, hamiltonian_type="zz_only")
av_energy_zz_only_iqm = np.mean(energy_zz_only_iqm, axis=0)

# Run simulation with only X terms (IQM)
print("Computing X terms only...")
energy_x_only_iqm = get_instances(state, echo=False, noise_model=iqm_noise_model, hamiltonian_type="x_only")
av_energy_x_only_iqm = np.mean(energy_x_only_iqm, axis=0)

# Calculate sum of Z and ZZ energies (IQM)
# av_energy_sum_iqm = av_energy_z_only_iqm + av_energy_zz_only_iqm

# Run simulation with full Hamiltonian (Z + ZZ + X) (IQM)
print("Computing full Hamiltonian...")
energy_full_iqm = get_instances(state, echo=False, noise_model=iqm_noise_model, hamiltonian_type="full")
av_energy_full_iqm = np.mean(energy_full_iqm, axis=0)
# Create DataFrame with energies for IQM device
data = {'time': ts}
data['energy_z_only_iqm'] = av_energy_z_only_iqm/L
data['energy_zz_only_iqm'] = av_energy_zz_only_iqm/L
data['energy_x_only_iqm'] = av_energy_x_only_iqm/L
# data['energy_sum_iqm'] = av_energy_sum_iqm/L
data['energy_full_iqm'] = av_energy_full_iqm/L

df = pd.DataFrame(data)
csv_filename = f"energy_comparison_iqm_{state}_g{g}_L{L}_inst{inst}_randomphi{args.randomphi}_delta{phi_delta}_amplitude{phi_amplitude}_tf{args.tf}.csv"
...
# Convert from wide format to long format for all Hamiltonian types
df_melted = df.melt(id_vars=['time'], 
                    value_vars=[col for col in df.columns if col.startswith('energy_')],
                    var_name='config', 
                    value_name='energy')

# Extract Hamiltonian type for IQM data
df_melted['hamiltonian_type'] = df_melted['config'].str.extract(r'energy_([^_]+(?:_[^_]+)?)_iqm')[0]
df_melted['device'] = 'IQM'
...
# Create plot for IQM device
print(f"\nCreating plot for IQM device")

# Create a single plot comparing all Hamiltonian types for IQM device
fig, ax = plt.subplots(1, 1, figsize=(5.7*2, 4.3*2))

# Data for Z terms only
data_z_only = df_melted[(df_melted['device'] == 'IQM') & (df_melted['hamiltonian_type'] == 'z_only')]
time_vals_z = data_z_only['time'].values
energy_vals_z = data_z_only['energy'].values

# Data for ZZ terms only
data_zz_only = df_melted[(df_melted['device'] == 'IQM') & (df_melted['hamiltonian_type'] == 'zz_only')]
time_vals_zz = data_zz_only['time'].values
energy_vals_zz = data_zz_only['energy'].values

# Data for X terms only
data_x_only = df_melted[(df_melted['device'] == 'IQM') & (df_melted['hamiltonian_type'] == 'x_only')]
time_vals_x = data_x_only['time'].values
energy_vals_x = data_x_only['energy'].values

# # Data for sum (Z + ZZ)
# data_sum = df_melted[(df_melted['device'] == 'IQM') & (df_melted['hamiltonian_type'] == 'sum')]
# time_vals_sum = data_sum['time'].values
# energy_vals_sum = data_sum['energy'].values

# Data for full Hamiltonian (Z + ZZ + X)
data_full = df_melted[(df_melted['device'] == 'IQM') & (df_melted['hamiltonian_type'] == 'full')]
...
ax.plot(time_vals_z, energy_vals_z, label='Z terms only ($\\sum_i h_i Z_i$)', 
        marker='o', markersize=5, linewidth=4, color=color_palette[0])
ax.plot(time_vals_zz, energy_vals_zz, label='ZZ terms only ($\\sum_i \\phi_i Z_i Z_{i+1}$)', 
        marker='s', markersize=5, linewidth=4, linestyle='-', color=color_palette[1])
ax.plot(time_vals_x, energy_vals_x, label='X terms only ($g\\pi \\sum_i X_i$)', 
        marker='d', markersize=5, linewidth=4, linestyle='-', color=color_palette[2])
ax.plot(time_vals_full, energy_vals_full, label='Z + ZZ + X (full)', 
        marker='v', markersize=5, linewidth=4, linestyle='-', color=color_palette[3])

ax.hlines(0, xmin=-5, xmax=T+10, colors='black', linestyles='--', linewidth=3)
...
ax.fill_between(time_vals_z, lower_env_z, upper_env_z, alpha=0.15, color=color_palette[0])
ax.fill_between(time_vals_zz, lower_env_zz, upper_env_zz, alpha=0.15, color=color_palette[1])
ax.fill_between(time_vals_x, lower_env_x, upper_env_x, alpha=0.15, color=color_palette[2])
ax.fill_between(time_vals_full, lower_env_full, upper_env_full, alpha=0.15, color=color_palette[3])

ax.set_xlabel('t (FT)', fontsize=14)
ax.set_ylabel(r'$\langle O \rangle$', fontsize=14)
title_str = f'IQM Garnet'
...
# Save the comparison plot for IQM device
plot_filename = f"hamiltonian_iqm_comparison_{state}_g{g}_L{L}_inst{inst}_tf{args.tf}.png"
plot_path = f"{folder_name}/{plot_filename}"
plt.savefig(plot_path, dpi=600, bbox_inches='tight')
print(f"Hamiltonian comparison plot for IQM device saved to {plot_path}")

plt.show()

print(f"\nCompleted IQM device simulation and plotting.")