...
def get_instances_adaptive_realtime(initial_state):
    ...
    Supports both linear and exponential feedback control
    """
    feedback_type = "exponential" if exponential_feedback else "linear"
    print(f"\nRunning real-time adaptive simulation with {feedback_type} feedback control...")
    print(f"Target echo: {target_echo}, Feedback gain: {feedback_gain}")
    if exponential_feedback:
    ...
    for i in range(inst):
        print(f"\nInstance {i+1}/{inst} (real-time adaptive control)")
        print("-" * 40)
        ...
        # Time evolution with real-time feedback
        for t in range(T):
    ...
    print(f"\nCompleted real-time adaptive simulation in {elapsed:.2f}s")
...
feedback_suffix = f"_exp{decay_compensation}" if exponential_feedback else "_linear"
csv_filename = f"autocorr_data_{state}_realtime_adaptive{feedback_suffix}_g{g_initial}_L{L}_inst{inst}_randomphi{args.randomphi}_delta{phi_delta}_amplitude{phi_amplitude}_noise{noise_prob}_usenoise{use_noise}_target{target_echo}_gain{feedback_gain}.csv"
...
comp_csv_filename = f"comparison_{state}_adaptive_vs_fixed_g{g_initial}_L{L}_inst{inst}_target{target_echo}_gain{feedback_gain}.csv"
...
# plt.tight_layout()
feedback_method = "Exponential" if exponential_feedback else "Linear"
plt.suptitle(f"Adaptive vs Fixed g Comparison L={L}, g∈[{g_min}, {g_max}]", y=0.98)

# Save the plot
plot_filename = f"adaptive_vs_fixed_g_comparison_{feedback_method.lower()}_{state}_L{L}_inst{inst}_target{target_echo}_gain{feedback_gain}.png"
