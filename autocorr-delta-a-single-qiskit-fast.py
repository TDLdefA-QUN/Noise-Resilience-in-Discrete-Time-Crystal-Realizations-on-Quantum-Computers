...
def qc_qiskit(initial_state, L, g, hs, phis, t, qubit, echo=False):
    ...
    # Run and get counts
    result = backend.run(circ_tnoise, shots=1024).result()
...
csv_filename = f"autocorr_data_{state}_g{g}_L{L}_inst{inst}_tf{args.tf}_randomphi{args.randomphi}_delta{phi_delta}_amplitude{phi_amplitude}_noise{noise_prob}_usenoise{use_noise}.csv"
