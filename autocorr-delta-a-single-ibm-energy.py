...
parser.add_argument("--L", type=int, default=127, help="Number of qubits")
...
folder_name = f"energy-data_L{L}-ibm"
...
disorder_folder = "."
hs_filename = os.path.join(disorder_folder, f"hs_L132_inst1_ampl1.0_delta0.0_randomphi1-first.csv")
phis_filename = os.path.join(disorder_folder, f"phis_L132_inst1_ampl1.0_delta0.0_randomphi1-first.csv")
...
def qc_qiskit(initial_state, L, g, hs, phis, t, qubit, echo=False, noise_model=None):
    ...
    snake_layout_energy = [
    19,18,14,0,1,2,3,4,15,
    5,6,7,8,16,
    9,10,11,12,13,
    17,30,31,32,36,51,50,49,55,48,47,46,
    35,28,29,
    27,26,25,24,34,
    23,22,21,20,33,39,40,
    38,37,52,56,57,58,71,
    59,60,61,
    53,41,42,43,44,45,54,63,
    64,65,66,73,
    67,68,69,70,74,89,88,87,93,
    86,85,84,83,92,
    82,81,72,
    62,
    80,79,91,
    78,77,76,75,90,94,95,96,109,
    97,98,99,100,110,
    101,102,103,104,111,
    105,106,107,108,112,126,125,124,123,122,121,120,119,118,117,116,115,114,113
]

    snake_layout = snake_layout_energy
    ...
    # layout_filename = f"{folder_name}/circuit_layout_t{t}_{echo_str}_opt{optimization_level}_{backend_name}_ibm.png"
...
def get_single_out(initial_state, inst_number, echo, noise_model=None):
    ...
    # Create CSV file for real-time saving
    csv_filename = f"energy_realtime_{initial_state}_g{g}_L{L}_inst{inst_number+1}_echo{echo}.csv"
    ...
    for t in range(T):
        print(f"Time step {t}, Instance {inst_number+1}")
        out = qc_qiskit(initial_state, L, g, hs[inst_number], phis[inst_number], t, int(L/2), echo=echo, noise_model=noise_model)
        results.append(out)
        
        # Save energy value immediately to CSV
        with open(csv_path, 'a') as f:
            f.write(f"{t},{out}\n")
        print(f"Saved energy {out} for time {t} to {csv_path}")
...
energy = get_instances(state, echo=False, noise_model=None)
...
for i, e in enumerate(energies):
    ...
    try:
        ...
        fit_ts = ts[0:]  # Skip t=0
        fit_e = e[0:]    # Skip corresponding energy value
