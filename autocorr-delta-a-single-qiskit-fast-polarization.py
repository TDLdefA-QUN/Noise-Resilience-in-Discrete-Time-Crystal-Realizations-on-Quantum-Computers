...
folder_name = f"autocorr_data_L{L}_polarization"
...
color_palette = ["#361AC1", "#15B300", "#D05A00", "#940EBD", "#0C8BCA", "#FF9100", "#E72142", '#AA4499']
...
def qc_qiskit(initial_state, L, g, hs, phis, t, qubit, polarization="x", echo=False):
    ...
    UF_subcircuit = create_UF_subcircuit(L, g, phis, hs, polarization)
    # Forward evolution
    for _ in range(t):
...
line_styles = ['-', '--', '-.', ':']
markers = ['o', 's', '^', 'D']  # Different markers for each polarization
...
for i, pol in enumerate(polarizations):
    ...
    def get_instances_with_pol(initial_state, pol, echo):
        ...
        all_results = []
        for j in range(inst):
            print(f"Instance {j+1}/{inst} ({'echo' if echo else 'forward'}) - {pol}", end="\r")
            results = []
            for t in range(T):
